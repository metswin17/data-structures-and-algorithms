...
function sortStarWarsCharacters(starWarsArr) {
...
function listFoods(recipe) {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    let foodItem = ingredient.slice(secondSpaceIndex + 1);
...
function splitFoods(recipe) {
  let result = [];
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let ingredient = recipe.ingredients[i];
    let parts = ingredient.split(' ');
    let foodItem = parts.slice(2).join(' ');
    result.push(foodItem);
  }
  return result;
}
...
function stepActions(recipe) {
  let result = [];
  for (let i = 0; i < recipe.steps.length; i++) {
    let step = recipe.steps[i];
    let firstSpaceIndex = step.indexOf(' ');
    let verb = step.slice(0, firstSpaceIndex);
    result.push(verb);
  }
  return result;
}
...
function extractVowels(str) {
  ...
  let consonants = str.replace(/[aeiouAEIOU]/g, '');
  return [consonants, vowels.sort().join('')];
}
